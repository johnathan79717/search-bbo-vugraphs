...
class Board < ActiveRecord::Base
  ...
  belongs_to :vugraph
  has_many :players_boards, dependent: :destroy
  has_many :players, through: :players_boards
  ...
  def parse_auction
    ...
    self.explanation = ''
    auctions = m['auction'].split(/\|+mb\|+/i)
    if auctions.size == 1
      auctions = auctions[0].scan(/[PDR]|[1-7](?:[CDHS]|NT?)/i)
    end
    auctions.map! do |bid|
      next if bid == '-'
  ...
  def parse(string, players, vugraph)
    ...
    @players.each do |name|
      ...
      self.players << Player.find_or_create_by(name: name.upcase)
    end
